  ...
  return (
    <div className="flex p-4 border-b dark:border-zinc-700 text-sm">
      <div className="flex-shrink-0 mr-3">
        <img className="w-10 h-10 rounded-full bg-zinc-200" src={user.profilePicture} alt={user.username} />
      </div>
      <div className="flex-1">
        <div className="flex items-center mb-1">
          <span className="font-bold mr-1 text-xs truncate">{user ? `@${user.username}` : "unkonwn user"}</span>
              ...
              </p>
              {
                currentUser && (currentUser._id === comment.userId || currentUser.isAdmin) && (
                  <>
                    ...
                    <button type="button" className="text-zinc-600 hover:text-orange-500" onClick={() => onDelete(comment._id)}>
                      Delete
                    </button>
                  </>
                )
